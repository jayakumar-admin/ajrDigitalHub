  ...
  template: `
    <div class="min-h-screen bg-slate-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div id="public-form-container" class="max-w-xl w-full space-y-8">
        ...
        <!-- Loading State -->
        @if (isLoadingForm()) {
          <div class="bg-white p-12 text-center rounded-3xl border border-slate-100 shadow-xl flex flex-col items-center justify-center gap-4">
            <div class="h-10 w-10 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin"></div>
            <p class="text-xs font-semibold text-slate-500 font-mono">Form loading...</p>
          </div>
        }

        <!-- Error State / Deactivated -->
        @else if (hasError() || (form() && !form()?.isActive)) {
          <div id="form-error-card" class="bg-white p-8 text-center rounded-3xl border border-rose-100 shadow-xl space-y-4">
            ...
            <h3 class="text-lg font-bold text-slate-900">Form Offline</h3>
        ...
        @else if (hasSuccessfullySubmitted()) {
          <div id="form-success-card" class="bg-white p-10 text-center rounded-3xl border border-emerald-100 shadow-2xl space-y-6 animate-fadeIn">
            ...
            <div>
              <h3 class="text-xl font-bold text-slate-900 font-sans">Form Received Successfully</h3>
              <p class="text-xs font-medium text-slate-400 mt-1">Submission Tally recorded in CRM.</p>
            </div>
            <p class="text-sm text-slate-500 max-w-sm mx-auto leading-relaxed">
            ...
            <div class="pt-2 border-t border-slate-50">
              <button 
                ...
                class="inline-flex items-center justify-center gap-1.5 px-4 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-semibold rounded-xl text-xs transition-all cursor-pointer"
              >
          ...
          <div class="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
            ...
            <div class="p-6 sm:p-10 space-y-6">
              ...
              <div class="space-y-2 border-b border-slate-100 pb-5">
                <h2 class="text-2xl font-bold tracking-tight text-slate-900 font-sans">{{ form()?.name }}</h2>
                <p class="text-xs text-slate-500 leading-relaxed">{{ form()?.description }}</p>
                <span class="inline-flex items-center text-[10px] uppercase font-mono tracking-wider font-bold text-slate-400">
              ...
              @if (toastMessage()) {
                <div class="p-3.5 rounded-xl flex items-center gap-2 border text-sm bg-rose-50 border-rose-100 text-rose-800">
              ...
              @if (dynamicForm) {
                <form [formGroup]="dynamicForm" (ngSubmit)="onSubmit()" class="space-y-6">
                  ...
                  @for (field of form()?.fields; track field.id) {
                    <div class="space-y-2">
                      <label [for]="field.id" class="block text-sm font-semibold text-slate-700 flex items-center gap-1 select-none">
                        {{ field.label }}
                        @if (field.required) {
                      ...
                      @if (field.type === 'text') {
                        <input 
                          ...
                          class="block w-full px-4 py-3 border border-slate-200 rounded-xl text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-slate-50/20"
                        >
                      }
                      ...
                      @if (field.type === 'dropdown') {
                        <select 
                          [id]="field.id"
                          [formControlName]="field.id"
                          class="block w-full px-4 py-3 border border-slate-200 bg-white rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-slate-50/20"
                        >
